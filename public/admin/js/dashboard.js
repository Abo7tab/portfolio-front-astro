...
async function renderCertificates(container) {
    ...
    if (certs.length === 0) {
        const empty = document.createElement('div');
        empty.className = 'alert alert-info';
        empty.textContent = 'No certificates added yet. Add your first certificate!';
        container.appendChild(empty);
        return;
    }
    ...
    certs.forEach(c => {
        ...
        // Title
        const title = document.createElement('h5');
        title.className = 'card-title fw-bold mb-1';
        title.textContent = c.title;
        body.appendChild(title);

        // Issuer
        const issuer = document.createElement('p');
        issuer.className = 'card-text text-muted small mb-1';
        issuer.innerHTML = `🏛️ ${c.issuer || '—'}`;
        ...
        if (c.date_acquired) {
            ...
            date.innerHTML = `📅 ${c.date_acquired}`;
...
async function renderProfile(container) {
    ...
    const profileImgSrc = info.profile_image
        ...
        : null;
    const cvUrl = info.cv_url
        ? (info.cv_url.startsWith('http') ? info.cv_url : appUrl + '/' + info.cv_url)
        : null;
    ...
    form.innerHTML = `
        <div class="row g-3">
            ...
            <!-- CV Upload -->
            <div class="col-12">
                <label class="form-label fw-bold">📄 CV / Resume (PDF)</label>
                ${cvUrl ? `<div class="mb-1"><a href="${cvUrl}" target="_blank" class="btn btn-sm btn-outline-success">📥 Current CV</a></div>` : ''}
                <input type="file" class="form-control" name="cv_file" accept=".pdf">
                <small class="text-muted">Upload a new PDF to replace the current CV</small>
            </div>
